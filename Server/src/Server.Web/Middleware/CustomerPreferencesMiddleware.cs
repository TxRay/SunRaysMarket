...
internal class CustomerPreferencesMiddleware(ICookieService cookieService, IServiceProvider serviceProvider)
    : IMiddleware
{
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        using var scope = serviceProvider.CreateScope();
        var customerPreferencesService = scope.ServiceProvider.GetRequiredService<ICustomerPreferencesService>();
        ...
        if (cookieService.WasCookieUpdated(cs => cs.Preferences))
        {
            await customerPreferencesService.SetCustomerPreferences(
                ...
                {
                    PreferredStoreId = cookieService.Preferences.PreferredStoreId ?? currentPreferences.PreferredStoreId!.Value
                }
            );
        }
    }
