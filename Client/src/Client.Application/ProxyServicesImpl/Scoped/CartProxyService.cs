...
public class CartProxyService(HttpClient client) : ICartService
{
    public async Task<IEnumerable<CartItemListModel>> GetCartItemsAsync(int cartId)
        => await client.GetFromJsonAsync<IEnumerable<CartItemListModel>>($"api/cart/items/{cartId}")
           ?? [];

    public async Task<IEnumerable<CartItemListModel>> GetActiveCartItemsAsync()
        => await client.GetFromJsonAsync<IEnumerable<CartItemListModel>>($"api/cart/items")
           ?? [];

    public async IAsyncEnumerable<CartItemListModel> GetActiveCartItemsAsyncEnumerable()
    {
        await foreach (var item in client.GetFromJsonAsAsyncEnumerable<CartItemListModel>("api/cart/items/streamed"))
        {
            if (item is not null)
                yield return item;
            else
