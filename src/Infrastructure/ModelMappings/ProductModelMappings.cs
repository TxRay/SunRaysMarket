...
{
    public static async Task<ProductDetailsModel?> ToProductDetailsAsync(this IQueryable<Product> set)
    => await set .Select(
            p =>
    ...
    public static async Task<IEnumerable<ProductListModel>> ToProductListAsync(this IQueryable<Product> set)
    => await set.Select(
        p =>
